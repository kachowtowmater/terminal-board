...
- `tb assign ID NAME` is `tb take` for someone else (TODO only, no `--force`); the log splits who assigned it from who now holds it. A board's rules (`tb config rules`) print with `tb guide` and show once, on your first `tb next` after they are set or changed.
