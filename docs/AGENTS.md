...
- Name your branch after the issue (`fix/315-flags`) or write `Closes #315` in the PR so the link
  is found; `gh#N` is case-insensitive (`GH#6`), and `tb sync` reports one matching nothing (`tb edit` it).
