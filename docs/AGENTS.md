...
| finished your work (DOING → REVIEW) | `tb done ID` — a verifier closes REVIEW cards (see below) |
| another agent holds the card you want to change | refused — `--force` if you mean it (logged); the TUI asks y/n |
| checked a card, not closing it (stays in REVIEW) | `tb done ID --approve` |
...
`tb move ID todo` clears the owner; sending REVIEW back needs a reason, keeps the owner and skips the WIP limit. Text from a file arrives byte for byte
(UTF-8, at most 256 KiB, empty refused; `-` reads a pipe and waits for it to close — `TB_STDIN_TIMEOUT` bounds the wait for its first byte). Board order:
`TB_DB` > a name on the command line > `TB_BOARD` > the saved default > `default`. Leave settings alone unless a person asks you to.
...
If the board is connected to a repo (`tb config github` prints it): `tb github` lists PRs, issues with state and owner, what merged today and main's
CI; `tb sync` applies GitHub evidence to the board now (the 20 newest open PRs/issues, `tb github` shows the rest; `tb sync` also finds a taken
card's PR beyond that page).
...
- Name your branch after the issue (`fix/315-flags`) or write `Closes #315` in the PR. `gh#N` is case-insensitive; a `gh#N` that matches nothing is edited, not forced.
- `tb done` will not move a `gh#N` card to DONE while its issue is open: close the issue (or merge the PR) instead of adding `--force`.
