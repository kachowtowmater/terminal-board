...
| `unregistered_verifier` | REVIEW -> DONE by an agent whose close rode on a verifier role but whose resolved session has no entry in the verifier registry (`~/.local/state/terminal-board/verifiers/<session>`, written by `tb-agent-start --role verifier`), or whose entry's `name`/`harness` (claude-code) does not match this command; `TB_ROLE=verifier` from the environment alone never suffices. `TB_VERIFIER_REGISTRY=off` turns the check off; `--force` gets past it, logged |
