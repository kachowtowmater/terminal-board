...
| `events[]` | `{ts, actor, kind, text}` | the last 10, oldest first. Kinds include `created`, `taken`, `moved`, `note`, `check`, `blocked`, `unblocked`, `dropped`, `edit`, `prio`, `github`, `approved`; the set is open — see the forward-compatibility rule above and docs/SCHEMA.md |
