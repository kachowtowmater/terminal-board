...
fn review_to_todo_prints_a_todo_line() {
    ...
    assert!(out.contains(&format!("#{id} is back in TODO, unowned (round r2)")) && out.contains("is back in TODO, unowned") && out.contains(&format!("take {id}'")) && out.contains("anyone can take it"), "{out}");
