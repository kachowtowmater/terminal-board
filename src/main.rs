...
fn run(mut cli: Cli, positional: Option<String>) -> Result<(), BoardError> {
    ...
    if let Some(bad) = store.unknown_tz()? {
        warn!("tb: this board's tz '{bad}' is not a time zone this version knows — 'today' is taken from this machine's zone until you set it again: 'tb config tz America/Los_Angeles' (or 'tb config tz local')");
    }
    ...
    match cmd {
        ...
        Cmd::List => {
            ...
            if j {
                ...
                println!("{}", pretty(&snap.cards.iter().map(|c| snap.display.with(ctx.with(c, c), c)).collect::<Vec<_>>()));
            } else {
