...
//!     `TB_REAP_DEAD_AFTER` seconds (default 1800 = 30m) -> released to TODO with a note,
//!     via a direct library call (never the CLI's `--force` flag).
//!   - "idle" (no owner activity in >= the same window, owner still alive) is REPORTED only,
//!     never acted on.
...
use terminal_board::herdr::{self, AgentsState};
...
/// Does this card's owner have a live herdr agent?
fn agent_alive(card: &Card, fake: &Option<Vec<String>>) -> bool {
    let Some(owner) = card.owner.as_deref() else { return false };
    if let Some(names) = fake {
        return names.iter().any(|n| eq_ci(n, owner));
    }
    match herdr::probe() {
        AgentsState::Agents(agents) => herdr::exact_owner(&agents, card).is_some(),
        _ => false,
    }
}

/// Does this card's owner have a live tmux session named after them?
fn tmux_alive(card: &Card, fake: &Option<Vec<String>>) -> bool {
    let Some(owner) = card.owner.as_deref() else { return false };
    if let Some(names) = fake {
        return names.iter().any(|n| eq_ci(n, owner));
    }
    ...
    match out {
        Ok(o) if o.status.success() => {
            String::from_utf8_lossy(&o.stdout).lines().any(|l| eq_ci(l, owner))
        }
        _ => false, // no tmux server, or tmux not installed: no sessions are live
    }
}
...
fn scan_board(name: &str, mode: Mode, threshold: i64) -> Option<BoardResult> {
    ...
    let fake_agents = fake_list("TB_REAP_FAKE_AGENTS");
    let fake_tmux = fake_list("TB_REAP_FAKE_TMUX");
    let t = now();
    for c in cards.iter().filter(|c| c.column == "doing") {
        let age = t - c.column_since;
        let alive = agent_alive(c, &fake_agents) || tmux_alive(c, &fake_tmux);
        if !alive {
            if is_headless(&store, c.id) {
                // Headless workers never carry a pane or a tmux session by design — absence
                // of either is not evidence of death. Only note-staleness could flag one as
                // idle, and even then it is reported, never released.
                continue;
            }
            if age >= threshold {
            ...
            // owner is alive but the card has sat untouched for the same window: idle,
            // reported only.
    ...
    if mode == Mode::Apply {
        ...
        for (id, owner) in &dead_owner_cards {
            ...
            let reason = format!(
                "tb-reap: released — owner '{owner}' had no herdr agent and no tmux session for >= {}s",
                threshold
            );
...
fn main() {
    ...
    let results: Vec<BoardResult> = boards_list.iter().filter_map(|b| scan_board(b, mode, threshold)).collect();
